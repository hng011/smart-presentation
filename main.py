...
def main():
    ...
    while cap.isOpened():
        ...
        cv2.line(frame, (0, G_THRESHOLD), (WIDTH_CAM, G_THRESHOLD), (0,0,255), 6)
        ...
        fs_wnd= win32gui.GetWindowText(win32gui.GetForegroundWindow()).replace(" ", "").split("-")[-1]
        cv2.putText(detector, f"current window: {fs_wnd}", (5, HEIGHT_CAM), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,0), 2)
        
        print("curr window: ",fs_wnd)
        cv2.putText(detector, f"log: {btn_log}", (5, HEIGHT_CAM+20), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,0), 2)
